...
/* ─── Main Component ────────────────────────────────────────────── */
const CorrectionsTable = ({ data, history, execID, sutType, fetchData, showNotification, isEditable, tester }) => {
  const currentUsername = sessionStorage.getItem("username");
  const currentEmail = sessionStorage.getItem("emailid");

  // Determine if the record is assigned to the currently logged-in user
  const isAssignedToMe = !tester || 
    (currentUsername && String(tester).toLowerCase() === currentUsername.toLowerCase()) || 
    (currentEmail && String(tester).toLowerCase() === currentEmail.toLowerCase());

  const isReadOnly = sessionStorage.getItem("is_read_only") === "true" || !isAssignedToMe;
    ...
    <>
      <Stack gap={0.5}>
        ...
        {data.map((group, groupIdx) => {
          ...
          const isActuallyEditable = isEditable && isPending && !isReadOnly;
          const isAccepted =
