...
async def build_expertise_query_filter(expert_cats: list, db, username: Optional[str] = None, email: Optional[str] = None, assigned_only: bool = False) -> dict:
    """
    Builds a MongoDB query filter for Executioninfo records strictly based on raw benchmarkCategory.
    If assigned_only is True, filters to records assigned to the specified user (by tester or assignment.assigned_sme).
    """
    if not expert_cats:
        return {}
    ...
    for cat in expert_cats:
        ...
        cat_conditions.append({"benchmarkCategory": cat_regex})

    if not cat_conditions:
        return {}

    cat_filter = {"$or": cat_conditions} if len(cat_conditions) > 1 else cat_conditions[0]

    if assigned_only and (username or email):
        # Match records where tester or assigned_sme is the user (by username or email)
        assignee_matches = []
        if username:
            assignee_matches.extend([{"tester": username}, {"assignment.assigned_sme": username}])
        if email:
            assignee_matches.extend([{"tester": email}, {"assignment.assigned_sme": email}])
        return {"$and": [cat_filter, {"$or": assignee_matches}]}
...
def get_current_user(request: Request) -> dict:
    ...
    if not auth_header or not auth_header.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Authorization header required. Please log in.")
