...
async def get_snapshot_records(Execution_id: str, user: dict = Depends(get_current_user)):
    ...
    # Try finding by benchmarkExecutionID first, then fallback to _id if it's a valid ObjectId
    exec_meta = await db[EXECUTION_INFO_COL].find_one({"benchmarkExecutionID": Execution_id})
    
    if not exec_meta and len(Execution_id) == 24:
        try:
            exec_meta = await db[EXECUTION_INFO_COL].find_one({"_id": ObjectId(Execution_id)})
            if exec_meta:
                # Update Execution_id to the actual benchmarkExecutionID for consistency in filters
                Execution_id = exec_meta.get("benchmarkExecutionID", Execution_id)
        except Exception:
            pass
            
    if not exec_meta:
        raise HTTPException(status_code=404, detail=f"Execution record {Execution_id} not found in master metadata.")
    ...
    if user.get("role", "").upper() == "SME":
        ...
        exp_filter = await build_expertise_query_filter(expert_cats, db, username=user.get("username"), email=user.get("email"))
        if exp_filter:
    ...
    if not doc or not doc.get("data"):
        raise HTTPException(
            status_code=404,
            detail=f"No snapshot record found for Execution_id: {Execution_id}"
        )
