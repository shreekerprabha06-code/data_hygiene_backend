...
# ============================================================================
# ELIGIBLE USERS & RECORD REASSIGNMENT
# ============================================================================

from app.schemas.models import ReassignRecordsRequest

@router.get("/eligible-users")
@router.get("/eligible-users/")
async def get_eligible_users(user: dict = Depends(get_current_user)):
    """
    Returns a list of all active SMEs and Admins who share at least one expertise category
    with the requesting user.
    """
    db = get_db()

    # 1. Fetch the latest expertise for the requesting user from the DB
    req_username = user.get("username")
    req_email = user.get("email")
    
    db_user = await db["users"].find_one({
        "$or": [{"username": req_username}, {"email": req_email}]
    })
    
    if not db_user:
        return {"status": "error", "message": "Requesting user not found in database"}

    # Use 'benchmarkCategories' as primary, fallback to 'expertise'
    ...
    if not my_expertise:
        return {
            "status": "success",
            "message": "No expertise defined for current user. No matches found.",
            "eligible_users": []
        }

    # Fetch all active SMEs and Admins
    users_cursor = db["users"].find(
        {"role": {"$in": ["SME", "Admin", "ADMIN", "sme"]}, "status": {"$nin": ["Inactive", "inactive"]}},
        {"_id": 0, "password": 0}
    )
    all_users = await users_cursor.to_list(length=200)
    ...
    for u in all_users:
        ...
        # Intersection check
        has_overlap = any(cat in my_expertise for cat in u_cats)
    ...
    return {
        "status": "success",
        "logged_in_user": req_username,
        "eligible_users": matches
    }


@router.post("/reassign-records")
async def reassign_records(req: ReassignRecordsRequest, user: dict = Depends(get_current_user)):
    """
    Reassigns execution records from one user to another.
    Validates that the target user has expertise matching the record's benchmarkCategory.
    Only Admins can reassign records.
    """
    db = get_db()
    ...
    if user.get("role", "").upper() != "ADMIN":
        raise HTTPException(
            status_code=403,
            detail="Access Denied: Only Admins can reassign records."
        )
    ...
    for item in req.assignments:
        e_id = item.e_id.strip()
        assign_to = item.assign_to.strip()

        # 1. Fetch the execution record
        exec_record = await db[EXECUTION_INFO_COL].find_one({"benchmarkExecutionID": e_id})
        if not exec_record:
            errors.append({
                "e_id": e_id,
                "status": "error",
                "message": f"Execution record {e_id} not found."
            })
            continue

        record_category = exec_record.get("benchmarkCategory", "")
        ...
        if not target_user:
            errors.append({
                "e_id": e_id,
                "status": "error",
                "message": f"User '{assign_to}' not found."
            })
            continue

        # 3. Validate expertise match
        target_cats = [str(c).strip().lower() for c in target_user.get("benchmarkCategories", target_user.get("expertise", []))]
        target_username = target_user.get("username", assign_to)

        if record_category and record_category.lower() not in target_cats:
            errors.append({
                "e_id": e_id,
                "status": "invalid assignment",
                "message": f'The user "{target_username}" is not expertise in "{record_category}" to assign the record "{e_id}"'
            })
            continue

        # 4. Update the tester and assignment fields
        target_email = target_user.get("email", assign_to)
        await db[EXECUTION_INFO_COL].update_one(
            {"benchmarkExecutionID": e_id},
            {"$set": {
                "tester": target_email,
                "assignment": {
                    "status": "ASSIGNED",
                    "assigned_sme": target_email,
                    "assigned_at": datetime.utcnow().isoformat(),
                    "reassigned_by": user.get("username"),
                    "reassigned_from": item.assign_from.strip(),
                    "proposals": [],
                    "feedback": None
                }
            }}
        ...
        results.append({
            "e_id": e_id,
            "status": "success",
            "message": f"Record {e_id} reassigned from {item.assign_from} to {target_email}"
        })
