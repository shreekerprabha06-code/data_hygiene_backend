...
async def get_invalid_summary(
    ...
    expertise: Optional[str] = Query(None, description="Comma-separated list of benchmark categories to filter by (e.g. 'Crypto,SPEC')"),
    ...
    # 2. Base Filters (Intersection of Search and Expertise)
    base_filters = []
    
    if expertise:
        # 1. Explicit expertise query parameter overrides token-based expertise
        cats = [c.strip() for c in expertise.split(",") if c.strip()]
        exp_filter = await build_expertise_query_filter(cats, db, username=user.get("username"), email=user.get("email"), assigned_only=is_assigned_only)
        if exp_filter:
            base_filters.append(exp_filter)
    elif role == "SME":
        # 2. SMEs are restricted to their own expertise by default
        expert_cats = user.get("benchmarkCategories", user.get("expertise", []))
        exp_filter = await build_expertise_query_filter(expert_cats, db, username=user.get("username"), email=user.get("email"), assigned_only=is_assigned_only)
        if exp_filter:
            base_filters.append(exp_filter)
    elif role == "ADMIN" and is_assigned_only:
        # 3. Admins only see assigned if requested
        assignee_matches = []
        ...
        if assignee_matches:
            base_filters.append({"$or": assignee_matches})
    ...
    if search:
        ...
        if is_uuid:
            base_filters.append({"benchmarkExecutionID": search})
        else:
            ...
            base_filters.append({"$or": or_filters})

    # Combine all base filters into one query object
    search_query = {}
    if len(base_filters) > 1:
        search_query = {"$and": base_filters}
    elif len(base_filters) == 1:
        search_query = base_filters[0]
    ...
    if search_query:
        # Merge search/expertise filters into the match_query
        for k, v in search_query.items():
            if k in match_query and k == "$and":
                # Merge AND arrays if they both exist
                match_query[k].extend(v)
            else:
                match_query[k] = v
